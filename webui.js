...
var utWebUI = {
        ...
        try {
            new Request.JSON({
                ...
                onFailure: function() {
                    ...
                    self.request2.delay(h * 1000, self, [ method, params, function(i) {
                        if (fails[0]) {
                            fails[0] = 0;
                            log("Request retry succeeded: " + method);
    ...
    getSettings: function(b) {
        ...
        this.request2("webui.getSettings", [], a)
    },
    ...
    showProperties: function(k) {
        ...
        if (this.propID != "multi") {
            this.request2("webui.getProperties", [ this.propID ], this.loadProperties)
        } else {
    ...
    getPeers: function(hash, forceLoad) {
        if (!hash) { hash = this.torrentID; }
        if (!hash) { return; }

        var now = Date.now();

        if (forceLoad || this.peerlist._ID_ !== hash || !this.peerlist._TIME_ || (now - this.peerlist._TIME_) > (this.limits.minPeerListCache * 1000)) {
            this.request2("webui.getPeers", [ hash ], (function(result) {
                this.peerlist.empty();
    ...
    getFiles: function(hash, forceLoad) {
        if (!hash) { hash = this.torrentID; }
        if (!hash) { return; }

        var now = Date.now();

        if (forceLoad || this.filelist._ID_ !== hash || !this.filelist._TIME_ || (now - this.filelist._TIME_) > (this.limits.minFileListCache * 1000)) {
            this.request2("webui.getFiles", [ hash ], (function(result) {
                this.filelist.empty();
